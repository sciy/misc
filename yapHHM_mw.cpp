...
//  gsl_odeiv2_driver * d = gsl_odeiv2_driver_alloc_y_new (&sys, gsl_odeiv2_step_rk4,   1e-3, 1e-8, 1e-8);
    ...
    cout << t << "  " << y[0] << "  " << y[1] << "  " << y[2] << "  " << y[3] << "  " << y[4] <<  "  " << g_Na*y[2]*y[2]*y[2]*y[4] << "  " << g_K*y[3]*y[3]*y[3]*y[3] << endl;
    fout << t << "  " << y[0] << "  " << y[1] << "  " << y[2] << "  " << y[3] << "  " << y[4] <<  "  " << g_Na*y[2]*y[2]*y[2]*y[4] << "  " << g_K*y[3]*y[3]*y[3]*y[3] << endl;
    //}}}

    for (int i = 1; i <= round(ts); i++)// i is the number of current time step{{{
    {
        double ti = i / tsfactor;
        if (i == (30 * tsfactor    )) y[0] = 7.3; // i == x*tsfactor: y[0]=y: turn the input voltage to y at time x
...
// //      gsl_odeiv2_driver_apply_fixed_step ( d,  t, h, n, y[]) evolves the driver system d from t with n steps of size h.
    ...
    plotter << "plot \"yapHHM_MembraneVoltage.txt\" u 1:5 w l lc 7  lw 2  pt 7  ps 1 t \"Na^{+}\", \"yapHHM_MembraneVoltage.txt\" u 1:6 w l lc 8  lw 2  pt 8  ps 1 t \"K^{+}\"" << endl;//}}}
